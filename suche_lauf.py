#!/usr/bin/env python3
"""Prüfstand – Suchlauf in der GitHub Action (Verfassung V3.4, E9).
...
Ohne neue Hypothesen wird nichts abgelegt (derselbe Datenstand ergibt dasselbe Ergebnis).
...
if zus["kandidaten_neu"] == 0:
    print("Keine neuen Hypothesen seit dem letzten Suchlauf: nichts abgelegt."); sys.exit(0)
...
def fmt(r):
    return (f"{r['indikator']} {r['art']} -> {r['ziel']} {r['h']} Tage: t {r['t']:.2f}, n {r['n']}, "
            f"{r['mu'] - r['mu0']:.2f} pp je Wechsel brutto, Hälften t {r['t1']:.2f} und {r['t2']:.2f}")
...
fund = bool(zus["bausteine"]) and zus["echt_mehr_als_staerkster_placebo"]
...
if zus["bausteine"] and not fund:
    urteil += " Überlebende vorhanden, aber nicht mehr als im stärksten Placebo-Lauf: Zufall nicht ausgeschlossen."
