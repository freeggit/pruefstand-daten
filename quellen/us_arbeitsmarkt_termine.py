#!/usr/bin/env python3
"""US-Arbeitsmarktbericht (Employment Situation) Veroeffentlichungstage: 1 am
Tag der monatlichen BLS-Veroeffentlichung, sonst 0. bls.gov selbst ist aus
dieser Umgebung mit HTTP 403 gesperrt (siehe Katalog bls_veroeffentlichungstage,
...
Terminkalender fuer Principal Federal Economic Indicators (u.a. Employment
Situation); fuer die Jahre davor ist eine vergleichbare, im Voraus bekannte
Ankuendigungspraxis nicht belegt, daher hier nicht verwendet (Zusatz 5: keine
historischen Termine ohne Beleg fuer vorherige Bekanntheit)."""
import datetime
import gzip
import io
import json
...
URL = "https://alfred.stlouisfed.org/release/downloaddates?rid=50&ff=txt"
...
def main():
    ...
    gzip_write(os.path.join(OUT_DIR, "arbeitsmarkt.csv.gz"), rows)

    meta = {
        "arbeitsmarkt": {
            "einheit": "Indikator (0/1)",
            "beschreibung": "1 am Veroeffentlichungstag des monatlichen US-Arbeitsmarktberichts (Employment Situation, BLS), sonst 0, fuer jeden Kalendertag ab 1985-01-01 bis heute. Termine aus dem ALFRED-Archiv der tatsaechlichen Veroeffentlichungstage dieses Release (rid=50).",
            "quelle_url": "https://alfred.stlouisfed.org/release/downloaddates?rid=50&ff=txt (Terminarchiv, ALFRED/Federal Reserve Bank of St. Louis) und https://www.bls.gov/ces/ (Urheber-Release, aus dieser Umgebung mit HTTP 403 gesperrt, siehe Katalogeintrag bls_veroeffentlichungstage)",
